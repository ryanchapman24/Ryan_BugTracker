...
namespace Ryan_BugTracker.Controllers
{
    ...
    public class HomeController : UserNames
    {
        ...
        public ActionResult Index(int? id)
        {
            ...
            {
                ViewBag.OpenTickets = db.Tickets.Where(t => t.TicketStatusId != 5).OrderByDescending(t => t.Created).ToList();
                ViewBag.ClosedTickets = db.Tickets.Where(t => t.TicketStatusId == 5).OrderByDescending(t => t.Created).ToList();
            }
            else if (User.IsInRole("Project Manager") || User.IsInRole("Developer"))
            {
                ...
                ViewBag.OpenTickets = userTickets.Union(projectTickets).Where(t => t.TicketStatusId != 5).OrderByDescending(t => t.Created).ToList();
                ViewBag.ClosedTickets = userTickets.Union(projectTickets).Where(t => t.TicketStatusId == 5).OrderByDescending(t => t.Created).ToList();
            }
            else
            {
                ViewBag.OpenTickets = db.Tickets.Where(t => t.AuthorUserId.Equals(uId)).Where(t => t.TicketStatusId != 5).OrderByDescending(t => t.Created).ToList();
                ViewBag.ClosedTickets = db.Tickets.Where(t => t.AuthorUserId.Equals(uId)).Where(t => t.TicketStatusId == 5).OrderByDescending(t => t.Created).ToList();
            }
